...
public class ConvolveDoubleBlocked
{
	private final int n;
	...
	public ConvolveDoubleBlocked( final Kernel1D[] kernels )
	{
	...
	{
		double[] previousTarget = source;
		for ( int d = 0; d < n; ++d )
		{
			final Kernel1D kernel = kernels[ d ];
			if ( kernel != null )
			{
				double[] currentTarget = ( d == lastNonNullKernel ) ? target : targets[ d ];
				convolve(
						previousTarget,
						currentTarget,
