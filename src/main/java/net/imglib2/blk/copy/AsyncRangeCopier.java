...
import net.imglib2.cache.img.AsyncCacheAsCacheAdapter;
...
class AsyncRangeCopier< T >
{
	...
	private final RandomAccess< CompletableFuture< Cell< Object > > > asyncCellAccess;

	private < T extends NativeType< T >, A >
	RandomAccess< CompletableFuture< Cell< A > > > getAsyncCellAccess( final AbstractCellImg< ?, ?, ? extends Cell< ? >, ? > cellImg )
	{
		if ( cellImg instanceof CachedCellImg )
		{
			final CachedCellImg< T, A > cachedCellImg = ( CachedCellImg< T, A > ) cellImg;
			final Cache< Long, Cell< A > > cache = cachedCellImg.getCache();
			if ( cache instanceof AsyncCacheAsCacheAdapter )
			{
				final Cache< Long, CompletableFuture< Cell< A > > > asyncCache = ( ( AsyncCacheAsCacheAdapter< Long, Cell< A > > ) cache ).getAsyncCache();
				final LazyCellImg.LazyCells< CompletableFuture< Cell< A > > > asyncCells = new LazyCellImg.LazyCells<>( cellImg.getCellGrid().getGridDimensions(), asyncCache.unchecked()::get );
				final RandomAccess< CompletableFuture< Cell< A > > > longListRandomAccess = asyncCells.randomAccess();
				return longListRandomAccess;
			}
		}
		return null;
	}
			...
			final T oob )
	{
		...
		asyncCellAccess = getAsyncCellAccess( cellImg );
	...
	public void copy( final int[] srcPos, final T dest, final int[] size )
	{
		...
		// copy data
		setupDestSize( size );
		if ( asyncCellAccess != null )
			primeAsync( n - 1 );
		copy( dest, n - 1 );
	}

	private void primeAsync( final int d )
	{
		for ( Ranges.Range range : rangesPerDimension[ d ] )
		{
			ranges[ d ] = range;
			asyncCellAccess.setPosition( ranges[ d ].gridx, d );
