...
import net.imglib2.algorithm.blocks.convert.ConvertScalars;
...
public class TransformPlayground3D_U8
{
...
//		final String fn = "/Users/pietzsch/workspace/data/leafcrop.tif";
		...
		final PrimitiveBlocks< UnsignedByteType > blocks = PrimitiveBlocks.of(
				Views.extendZero( img ) );
		Affine3DBlockProcessor_U8 processor = new Affine3DBlockProcessor_U8( affine.inverse() );
	...
	static class Affine3DBlockProcessor_U8 implements BlockProcessor< byte[], byte[] >
	{
		...
		private Supplier< Affine3DBlockProcessor_U8 > threadSafeSupplier;

		public Affine3DBlockProcessor_U8( final AffineTransform3D transformToSource )
		{
			...
			tempArray = TempArray.forPrimitiveType( PrimitiveType.BYTE );
		}

		private Affine3DBlockProcessor_U8( final Affine3DBlockProcessor_U8 affine )
		{
		...
		private Affine3DBlockProcessor_U8 newInstance()
		{
			return new Affine3DBlockProcessor_U8( this );
		}

		@Override
		public synchronized Supplier< Affine3DBlockProcessor_U8 > threadSafeSupplier()
		{
		...
		@Override
		public void compute( final byte[] src, final byte[] dest )
		{
			...
			for ( int z = 0; z < destSize[ 2 ]; ++z )
			{
				...
				for ( int y = 0; y < destSize[ 1 ]; ++y )
				{
					...
					for ( int x = 0; x < destSize[ 0 ]; ++x )
					{
						...
						final float a000 = ConvertScalars.from_u8( src[ o ] );
						final float a001 = ConvertScalars.from_u8( src[ o + 1 ] );
						final float a010 = ConvertScalars.from_u8( src[ o + ss0 ] );
						final float a011 = ConvertScalars.from_u8( src[ o + ss0 + 1 ] );
						final float a100 = ConvertScalars.from_u8( src[ o + ss1 ] );
						final float a101 = ConvertScalars.from_u8( src[ o + ss1 + 1 ] );
						final float a110 = ConvertScalars.from_u8( src[ o + ss1 + ss0 ] );
						final float a111 = ConvertScalars.from_u8( src[ o + ss1 + ss0 + 1 ] );
						dest[ i++ ] = ConvertScalars.to_u8( a000 +
								r0 * ( -a000 + a001 ) +
								r1 * ( ( -a000 + a010 ) +
